...
export default function PaymentOptions({ plan, method, setMethod }: Props) {
  ...
  const planExists = useMemo(() => {
    ...
    if (!ok) {   
      ...
      return !!plan && typeof plan.priceLabel === "string" && isValidUrl(targetUrl);
    }
  ...
  const handlePayment = async (methodName: string) => {
    ...
    try {
      ...
      router.push(
             `/betalmodeller/${plan.slug}/orderbekraftelse?method=${method.id.toLowerCase()}`
      )
